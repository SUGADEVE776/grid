...
from common.config import API_RESPONSE_ACTION_CODES
...
class AppAPIView(AppViewMixin, APIView):
    ...
    def get_object(self, exception=NotFound, identifier="pk"):
        ...
        if self.get_object_model:
            if _object := self.get_object_model.objects.get_or_none(
...
class CatalogueFilterMixin:
    ...
    def get_queryset(self):
        ...
        if catalogue_id := self.request.query_params.get("catalogue_id"):
            ...
            queryset = queryset.filter(related_learning_catalogues=catalogue).order_by(
                "created_at"
            )
        return queryset
    ...
    def get_queryset(self):
        ...
        if self.request.query_params.get("is_favourite"):
            queryset = queryset.filter(
                related_user_favourites__user=self.get_user()
            ).order_by("created_at")
        return queryset
